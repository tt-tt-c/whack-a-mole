...
export type MoleAlias = {
  play: Function;
};

export default class Mole extends React.Component<Props> {
  private mole: SpriteSheet | null;
  private isPopping: boolean = false;
  private isFeisty = false;
  private isHealing: boolean = false;
  private isWhacked: boolean = false; //叩かれた
  private isAttacking: boolean = false; //攻撃した

  private play: Function = () => {};
  private actionTimeout: NodeJS.Timeout | null = setTimeout(() => {});
  ...
  set setMole(mole: SpriteSheet | null) {
    this.mole = mole;
  }
  ...
  set setIsPopping(isPopping: boolean) {
  ...
  whack = () => {
    ...
    if (!this.isPopping || this.isWhacked || this.isAttacking) {
      //非表示 or 叩かれてる or 攻撃してきた 場合はreturn
      return;
    }
